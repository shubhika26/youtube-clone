...
console.log(this.state.list)
this.setState({
  currentVideoUrl: this.state.list[0].id.videoId
})
...
likeButton = () => {
  ...
  render() {
    let videos = this.state.list.map(eachVideo => (
...
{this.state.isLoadingError ? (<h1>No search found</h1>): (
  <iframe src={`https://www.youtube.com/embed/${this.state.currentVideoUrl}`} style={{height: '350px', width: '850px', float : 'left'}}/>
    ...
    <button  style={ {
  marginLeft: "790px" ,backgroundColor:" red",padding:'12px'}}onClick={this.likeButton}>{this.state.status}</button>
